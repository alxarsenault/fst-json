...
namespace json {
    ...
    public:
        ...
        inline node_const_ref get(std::size_t index) const
        {
            return node_const_ref(
                index, id[index], value[index], type[index], children[index]);
        }

        inline node_ref operator[](std::size_t index) { return get(index); }

        inline node_const_ref operator[](std::size_t index) const
        {
            return get(index);
        }
